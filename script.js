...
if (heroLogoVideo && heroLogoFallback) {
  const showFallbackLogo = () => {
    heroLogoVideo.style.display = "none";
    heroLogoFallback.style.display = "block";
  };

  const hideFallbackLogo = () => {
    heroLogoVideo.style.display = "block";
    heroLogoFallback.style.display = "none";
  };

  const supportedFormats = ["video/mp4", "video/webm"];
  const hasSupportedVideoFormat = supportedFormats.some((type) => heroLogoVideo.canPlayType(type) !== "");

  if (heroLogoVideo.canPlayType && !hasSupportedVideoFormat) {
    showFallbackLogo();
  }

  heroLogoVideo.addEventListener("error", showFallbackLogo);
  heroLogoVideo.addEventListener("loadeddata", () => {
    if (heroLogoVideo.videoWidth && heroLogoVideo.videoHeight) {
      hideFallbackLogo();
    } else {
      showFallbackLogo();
    }
